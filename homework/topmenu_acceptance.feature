Feature: Demo Nopcommerce's topmenu acceptance criteria
  ...
  Background: User is on given URL
    When  User type URL htpps://demo.nopcommerce.com
  ...
  Scenario: Verify that user can see 'Grid' tab uon Books Category page
    Given User is on Books Category page
    When User is on Books Category page
    Then Check 'Grid' tab is present
  ...
  Scenario: Verify that User can see 'Name A to Z' is first option in 'Sort by' filter
    Given User is on Books Caterogy page
    When User is on Books Category page
    And Click on 'Sort by' filter
